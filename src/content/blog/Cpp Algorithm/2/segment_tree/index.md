...
구간 합, 구간 최솟값, 구간 최댓값처럼 두 구간의 값을 합쳐서 더 큰 구간의 값을 만들 수 있을 때 사용할 수 있다.
...
그러면 `i`번 노드의 왼쪽 자식은 `i*2`, 오른쪽 자식은 `i*2+1`이 된다.
...
변경된 원소를 포함하는 구간만 갱신하면 되므로 $O(\log N)$에 처리할 수 있다.
...
초기화는 모든 노드를 한 번씩 채우므로 $O(N)$이다.

구간 쿼리와 값 변경은 트리의 높이만큼 이동하므로 각각 $O(\log N)$이다.
