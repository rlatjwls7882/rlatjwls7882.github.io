...
Trie는 여러 문자열을 공통 접두사 단위로 묶어 저장하는 자료구조이다.

문자열을 하나씩 저장하는 대신 같은 접두사를 공유하는 문자열은 같은 경로를 사용한다.

따라서 문자열의 길이에 비례하는 시간에 삽입과 탐색을 처리할 수 있다.
...
알파벳 소문자로 이루어진 문자열을 저장하는 Trie는 다음과 같이 구현할 수 있다.
...
문자열 삽입과 접두사 탐색은 문자를 한 번씩 확인하므로 $O(L)$에 처리할 수 있다.
