...
Line Intersection은 두 선분이 교차하는지 판별하는 알고리즘이다.

두 선분 `P1P2`, `P3P4`가 있을 때 CCW를 이용해 두 선분의 위치 관계를 확인한다.
...
먼저 선분 `P1P2`를 기준으로 `P3`, `P4`가 서로 다른 방향에 있는지 확인한다.
...
두 값의 부호가 다르면 `P3`, `P4`는 선분 `P1P2`의 서로 다른 쪽에 있다.

둘 중 하나가 `0`이면 한 점이 `P1P2`와 같은 직선 위에 있다는 뜻이다.

## 반대 방향도 확인

이번에는 선분 `P3P4`를 기준으로 `P1`, `P2`가 서로 다른 방향에 있는지 확인한다.
...
두 선분이 같은 직선 위에 있는 경우는 별도로 처리해야 한다.

![일직선 위에서 겹치는 상태](5.svg)

이때는 CCW 값들이 모두 `0`이 된다.

따라서 각 선분의 양 끝점을 정렬한 뒤 두 구간이 겹치는지 확인한다.
