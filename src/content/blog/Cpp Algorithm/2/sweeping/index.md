...
complexity: '정렬 $O(N\log N)$, 스캔 $O(N)$'
---

스위핑은 데이터를 정렬한 뒤 한 방향으로 훑으며 답을 구하는 알고리즘이다.

이 글에서는 선분이 차지하는 전체 길이를 구하는 문제를 예로 설명한다.
...
선분들을 합치면 $[1,7]$, $[8,10]$이 남고 전체 길이는 $8$이다.
...
$[2,7]$은 $[1,3]$과 겹치므로 현재 구간을 $[1,7]$로 늘린다.

$[4,6]$은 $[1,7]$ 안에 포함되므로 그대로 둔다.

## 겹치지 않는 구간

다음 선분의 시작점이 `right`보다 크면 현재 구간과 겹치지 않는다.

이때 현재 구간의 길이를 답에 더하고 새 구간을 시작한다.
...
따라서 $[1,7]$의 길이 $6$을 더하고 현재 구간을 $[8,10]$으로 바꾼다.

반복이 끝나면 마지막 구간의 길이도 더한다.
...
선분이 차지하는 전체 길이는 다음과 같이 구할 수 있다. $O(n \log n)$
...
정렬에 $O(n \log n)$이 걸리고 모든 선분을 한 번씩 확인한다.
