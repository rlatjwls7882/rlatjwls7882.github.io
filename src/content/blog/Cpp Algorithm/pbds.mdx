...
`ordered_set`은 `set`처럼 중복되지 않는 원소를 정렬된 상태로 저장한다. 여기에 특정 값보다 작은 원소의 개수와 정렬했을 때 `k`번째인 원소를 구하는 기능이 추가되어 있다.
...
`find_by_order()`는 정렬했을 때 `k`번째인 원소를 가리키는 이터레이터를 반환한다. $O(\log n)$
