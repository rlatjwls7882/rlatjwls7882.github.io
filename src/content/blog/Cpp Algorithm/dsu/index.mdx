...
`DSU`는 서로 겹치지 않는 집합을 관리하는 자료구조이며 `Union-Find`라고도 부른다.

두 원소가 같은 집합에 속하는지 확인하거나 두 집합을 하나로 합칠 때 사용한다.
...
작은 인덱스를 대표 원소로 선택해도 충분하다.

하지만 더 안정적인 시간복잡도를 보장하려면 크기가 작은 집합을 큰 집합 아래에 연결하면 된다.
