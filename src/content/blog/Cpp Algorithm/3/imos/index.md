...
Imos Method는 여러 구간에 값을 더한 뒤 최종 상태를 빠르게 구하는 방법이다.

구간의 모든 원소를 직접 바꾸지 않고 값이 변하기 시작하는 위치와 끝나는 위치만 기록한다.
...
Imos Method에서는 변화량 배열 `diff`에 두 값만 기록한다.
...
예를 들어 인덱스가 `2` 이상 `5` 이하인 원소에 `3`을 더한다고 하자.
...
각 갱신은 $O(1)$에 기록할 수 있다.

마지막에 누적 합을 한 번 구하므로 전체 시간복잡도는 $O(n+q)$이다.

## 기존 배열에 적용하기

기존 배열 `a`에 구간 갱신 결과를 적용하려면 각 위치에 `diff[i]`를 더하면 된다.
...
Imos Method는 2차원 배열에서도 사용할 수 있다.

행이 `x1` 이상 `x2` 이하이고 열이 `y1` 이상 `y2` 이하인 사각형 영역에 `x`를 더한다고 하자.
...
가로 방향과 세로 방향으로 누적 합을 구하면 실제 값이 복원된다.
...
이번에는 Imos Method를 이용해 한 변의 길이가 `4`인 직각 삼각형 영역을 만들어 보자.
...
먼저 배열을 왼쪽에서 오른쪽으로 훑으며 가로 방향 누적 합을 구한다.
