...
description: '증가 경로를 DFS로 찾아 \href{https://ko.wikipedia.org/wiki/이분_그래프}{이분 그래프}의 최대 매칭을 구하는 알고리즘'
complexity: '$O(VE)$'
---

`Kuhn` 알고리즘은 이분 그래프에서 최대 매칭을 구하는 알고리즘이다.

기존 매칭을 재배치할 수 있는지 `DFS`로 확인하며 새로운 매칭을 하나씩 추가한다.
...
`Kuhn` 알고리즘은 `DFS`를 이용해 이러한 재배치가 가능한지 확인한다.
...
bool dfs(int cur) {
    ...
    for(int nxt:conn[cur]) {
        if(!B[nxt] || !visited[B[nxt]] && dfs(B[nxt])) {
            A[cur]=nxt;
            B[nxt]=cur;
            return true;
        }
...
이미 매칭된 정점이라면 현재 연결된 왼쪽 정점을 다른 곳으로 옮길 수 있는지 확인한다.

`visited` 배열은 하나의 왼쪽 정점에서 시작한 탐색 안에서 같은 정점을 반복해서 확인하지 않도록 막는다.

새로운 왼쪽 정점을 확인할 때마다 초기화한다.

```cpp
memset(visited, 0, sizeof visited);
```

각 왼쪽 정점마다 `DFS`를 한 번씩 수행한다.

한 번의 `DFS`에서는 최대 $E$개의 간선을 확인하므로 시간복잡도는 $O(VE)$이다.
