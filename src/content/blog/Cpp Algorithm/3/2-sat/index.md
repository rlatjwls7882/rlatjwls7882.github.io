...
2-SAT은 각 절에 리터럴이 두 개씩 포함된 논리식이 주어졌을 때 식을 만족하는 값 할당이 존재하는지 판별하는 문제이다.
...
따라서 절 `(x ∨ y)`가 주어졌다면 다음 두 간선을 추가한다.
...
같은 SCC에 속한 리터럴은 서로 도달할 수 있다.

어떤 변수 `x`와 그 부정 `¬x`가 같은 SCC에 속한다면 `x`는 참이면서 동시에 거짓이어야 한다. 따라서 식을 만족시킬 수 없다.
...
변수 `x`는 `2x-1`번 정점에 저장하고 `¬x`는 `2x`번 정점에 저장한다.
...
Tarjan 알고리즘으로 SCC를 구하면 2-SAT을 $O(N+M)$에 판별할 수 있다.

변수는 `N`개이고 절은 `M`개라고 하자. Implication graph에는 정점 `2N`개와 간선 `2M`개가 생긴다.
