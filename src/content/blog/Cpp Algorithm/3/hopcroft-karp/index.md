...
Hopcroft-Karp는 이분 그래프에서 최대 매칭을 구하는 알고리즘이다.

Kuhn 알고리즘은 왼쪽 정점마다 DFS를 수행하지만 Hopcroft-Karp는 BFS로 레벨을 만든 뒤 DFS를 이용해 여러 증가 경로를 한 단계에서 처리한다.
...
Hopcroft-Karp는 BFS와 DFS를 반복한다.

BFS에서는 아직 매칭되지 않은 왼쪽 정점의 레벨을 `0`으로 두고 큐에 넣는다.
...
예를 들어 `cur → next → b[next]`로 이동했다면 다음 왼쪽 정점의 레벨은 현재 레벨보다 `1` 커진다.

BFS가 끝나면 DFS를 이용해 실제 매칭을 추가한다.

DFS에서는 오른쪽 정점 `next`가 비어 있다면 바로 매칭한다.
...
첫 번째 단계에서는 다음과 같은 매칭을 한 번에 추가할 수 있다.
...
아직 매칭되지 않은 왼쪽 정점이 남아 있다면 BFS를 다시 수행한다.
...
한 단계에서 새로운 매칭을 하나도 추가하지 못했다면 더 이상 증가 경로가 없어 종료한다.
...
일반적인 Hopcroft-Karp 알고리즘의 시간복잡도는 $O(E\sqrt V)$이다.

다만 위 구현은 코드를 간결하게 만들기 위해 최단 증가 경로의 길이를 별도로 제한하지 않고 이미 확인한 간선을 건너뛰는 최적화도 사용하지 않는다.

입력 크기가 크거나 최악 시간복잡도가 중요한 문제에서는 최단 증가 경로 제한과 현재 간선 최적화를 추가한 구현을 사용하는 편이 안전하다.
